...
public class MapGenerator : MonoBehaviour
{
    ...
    public Transform navmesMaskPrefab;
    ...
    public void GenerateMap()
    {
        ...
        for (int i = 0; i < obstacleCount; i++)
        {
            ...
            if (randomCoord != currentMap.mapCenter && MapIsFullyAccess(obstacleMap, currentObstacleCount))
            {
                ...
                Transform newObstacle = Instantiate(obstaclePrefab, obstaclePosition
                    + Vector3.up * obstacleHeight/2, Quaternion.identity) as Transform;
        ...
        shuffledOpenTileCoords = new Queue<Coord>(Utility.ShuffleArray(
            allOpenCoords.ToArray(), currentMap.seed));
        // create navmesh mask
        Transform maskLeft = Instantiate(navmesMaskPrefab,Vector3.left*
            (currentMap.mapSize.x + maxMapSize.x)/ 4.0f * tileSize, Quaternion.identity) as Transform;
        maskLeft.parent = mapHolder;
        maskLeft.localScale = new Vector3((maxMapSize.x - currentMap.mapSize.x) / 2.0f, 1, currentMap.mapSize.y) * tileSize;

        Transform maskRight = Instantiate(navmesMaskPrefab, Vector3.right *
            (currentMap.mapSize.x + maxMapSize.x) / 4.0f * tileSize, Quaternion.identity) as Transform;
        maskRight.parent = mapHolder;
        maskRight.localScale = new Vector3((maxMapSize.x - currentMap.mapSize.x) / 2.0f, 1, currentMap.mapSize.y) * tileSize;

        Transform maskTop = Instantiate(navmesMaskPrefab, Vector3.forward *
            (currentMap.mapSize.y + maxMapSize.y) / 4.0f * tileSize, Quaternion.identity) as Transform;
        maskTop.parent = mapHolder;
        maskTop.localScale = new Vector3(maxMapSize.x, 1, (maxMapSize.y - currentMap.mapSize.y) / 2.0f) * tileSize;

        Transform maskBottom = Instantiate(navmesMaskPrefab, Vector3.back *
            (currentMap.mapSize.y + maxMapSize.y) / 4.0f * tileSize, Quaternion.identity) as Transform;
        maskBottom.parent = mapHolder;
        maskBottom.localScale = new Vector3(maxMapSize.x, 1, (maxMapSize.y - currentMap.mapSize.y) / 2.0f) * tileSize;

        navmeshFloor.localScale = new Vector3(maxMapSize.x, maxMapSize.y) * tileSize;
        mapFloor.localScale = new Vector3(currentMap.mapSize.x * tileSize
            , currentMap.mapSize.y * tileSize);
    }

    bool MapIsFullyAccess(bool[,] obstacleMap, int currentObstacleCount)
    {
    ...
    Vector3 CoordToPosition (int x , int y)
    {
        ...
        return new Vector3(-currentMap.mapSize.x / 2.0f + 0.5f + x,
                    0, -currentMap.mapSize.y / 2.0f + 0.5f + y) *tileSize;
    }

    public Transform GetTimeFromPosition (Vector3 position)
    {
        ...
        x = Mathf.Clamp(x, 0, tileMap.GetLength(1)-1);
        return tileMap[x, y];
